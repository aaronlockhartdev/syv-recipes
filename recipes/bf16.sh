#!/bin/bash
# bf16 recipe: the dflash2 stack with the unquantized bf16 KV cache.
#
...
# clean), so this recipe does not force PIECEWISE like mtp.sh does.
#
# The trade vs dflash2.sh: bf16 KV is 2x the bytes of int8, so the same VRAM
...
if [ "${VLLM_OFFLOAD_KEEP_SHM:-0}" != 1 ]; then
  for f in /dev/shm/vllm_offload_*.mmap; do
    [ -e "$f" ] || continue
    grep -lqs "$f" /proc/[0-9]*/maps 2>/dev/null || { echo "[bf16] removing stale offload region $f"; rm -f "$f"; }
  done
fi
