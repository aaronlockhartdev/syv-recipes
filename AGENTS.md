...
- **The Swift variant is opt-in** (`SWIFT=1` in setup/prepare and in every recipe; `SWIFT=/path` redirects): `prepare/build_swift_model.py` builds the variant from ukisai's own W4A16 AWQ quant of the Swift 1.5 reasoning-efficiency fine-tune (58.5% fewer thinking tokens, +0.35% vs base on ukisai's benches). That export is already a loadable compressed-tensors format (LLM-Compressor-built: 4-bit asymmetric group-128, weight_packed/weight_scale/weight_zero_point), so there is no conversion pass -- the build applies the fast model's round-to-nearest operations (int8 group-128 lm_head/embed_tokens/MTP module), adds the three int8 config groups to the publisher's config (dropping lm_head from its ignore list), and installs the froggeric template. The repo's card declares it gated, but the HF API currently resolves it ungated: if a fetch is refused, accept the Swift Open License on the HF page and set HF_TOKEN. A truthy `SWIFT` makes the recipes
